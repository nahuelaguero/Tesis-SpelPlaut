...
export async function GET(request: NextRequest) {
  try {
    ...
    } else if (user.rol !== "admin" && user.cancha_id) {
      // Si es propietario, solo ver su cancha
      filtroBase.cancha_id = user.cancha_id;
    }
    ...
    const todasLasReservas = await Reserva.find(
      cancha_id
        ? { cancha_id }
        : user.rol !== "admin" && user.cancha_id
        ? { cancha_id: user.cancha_id }
        : {}
    );
